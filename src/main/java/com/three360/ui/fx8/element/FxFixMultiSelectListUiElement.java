...
public class FxFixMultiSelectListUiElement implements IFixMultiSelectListUiElement<Pane, String> {
    ...
    public void setValue(String s) {
        ...
        multiSelectListView.setSelectionModel();
        FXCollections.observableArrayList(this.multiSelectListT
                .getListItem()
                .stream()
                .filter(listItemT -> s.contains(listItemT.getEnumID()))
                .collect(Collectors.toList()));
        Arrays.asList(s.split(" ")).stream().map(s1 -> {
